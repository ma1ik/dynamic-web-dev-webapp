const CONFIG = {
    ...
    Flavours: ["Butter", "Caramel", "Birthday Cake", "Cheese", "Jalapeno", "Chocolate", "Sea Salt", "Sweet", "Ketchup", "Dorito", "S'mores", "White cheddar", "Buffalo Ranch", "Coconut", "Cookies and Cream", "Cinnamon"]
};
...
class MoviesApi {
    ...
    async searchQuery(movieSearch, shows = false){
        ...
        if (data !== false) {
            ...
            if (!shows) {
                DOMStuff.discoverInserter(data, "#moviesList", "movie");
            } else {
                DOMStuff.discoverInserter(data, "#showsList", "tv");
            }
        } else {
            return false;
        }
...
class DOMStuff {
    static discoverInserter(data, element, type) {
        const elem = $(element);
        ...
        if (data !== false) {
            for (let i = 0; i < data.results.length; i++) {
                ...
                to_insert.push(
                    $("<div/>", {class: "movieCard", "data-type": type, "data-id": result.id}).append(
                        ...
                        $("<div/>", {class: "movieInfo"}).append(
                            ...
                            $('<div class="top-right add-to-watched"><i class="fas fa-eye"></i></div>')
                        )
                    )
                );
            }
...
function makeClickable(show = false) {
    ...
    movieCard.on("click", function (event) {
        ...
        if (target.data("id") !== undefined) {
            window.location.href = "info?" + (show ? "show" : "movie") + "=" + target.data("id");
        }
    });

    addToWatched.off("click");
    addToWatched.on("click", async function (event) {
        event.preventDefault(); event.stopPropagation();

        let target = $(event.target);
        let image = target.parents("[data-type]").children("img")[0].src;
        let title = target.parents("[data-type]").children("div").children("h2")[0].innerText;

        let type = target.parents("[data-type]").data("type");
        let id = target.parents("[data-id]").data("id");

        try {
            await fetch("/like", {method: 'post', body: JSON.stringify({type, id, image, title}), headers: {"content-type": "application/json"}})
        } catch (ignored) {}
    });
}

function getRandomPopcorn(){
    var randomFlavor = CONFIG.Flavours[Math.floor(Math.random() * CONFIG.Flavours.length)];
    alert( randomFlavor);
}